...
class ForwardKinematics(object):
    ...
    """    
    def print_info(self, link_names, joints, joint_values):
        # Links
        for i in range(0, len(link_names)):
            print("Link " + link_names[i])
        # Joint
        for i in range(0, len(joints)):
            print("Join " + str(joints[i].name) + " axis " + str(joints[i].axis) + " type " + str(joints[i].type) + " origin xyz " + str(joints[i].origin.xyz) + " origin rpy " + str(joints[i].origin.rpy))
        # Joint values
    ...
    def compute_transforms(self, link_names, joints, joint_values):
        ...
        #self.print_info(link_names, joints, joint_values)


        for i in range(0, len(link_names)):
            if i == 0:
                temp = numpy.dot(tf.transformations.quaternion_matrix(tf.transformations.quaternion_from_euler(0.0, 0.0, 0.0)), tf.transformations.translation_matrix((joints[i].origin.xyz[0], joints[i].origin.xyz[1], joints[i].origin.xyz[2])))
                all_transforms.transforms.append(convert_to_message(temp, link_names[i], "world_link"))

            else:
                for s in range(0, len(joint_values.name)):
                    if joint_values.name[s] == joints[i].name:


                        temp = numpy.dot( 
                            tf.transformations.translation_matrix((
                                joints[i].origin.xyz[0], 
                                joints[i].origin.xyz[1], 
                                joints[i].origin.xyz[2])),
                            tf.transformations.quaternion_matrix(
                                tf.transformations.quaternion_from_euler(
                                    joint_values.position[s]*joints[i].axis[0], 
                                    joint_values.position[s]*joints[i].axis[1], 
                                    joint_values.position[s]*joints[i].axis[2]))
                            )


                        all_transforms.transforms.append(convert_to_message(temp, link_names[i], link_names[i-1]))
